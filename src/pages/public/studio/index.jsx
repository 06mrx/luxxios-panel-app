...
export default function Index() {
    ...
    const { data: datas, mutate, isLoading: datasLoading } = useSWR('/api/public/studio/index?page=' + (ROUTER.query.page ? ROUTER.query.page : ''), fetcher)
    const handleChangeFeedback = (val) => {
        fetch(process.env.API + '/api/public/studio/feedback/' + selected?.id, {
            method: 'POST',
            headers: {
                'authorization': TOKEN,
    ...
    return <>
        ...
        <NewPageLayout title={'Pendaftaran Sanggar'} hasBackURL backURL="/public">
            <div className="mt-3">
                <AddButton hasUrl url="/public/studio/create" />
                <Table>
                    <thead>
                            ...
                            datas?.data?.data?.map((data, index) => {
                                return (
                                    <tr key={index} className="hover text-center">
                                        ...
                                        <td>
                                            <ShowButton url={'/public/studio/' + data.id} />
                                        </td>
                                    </tr>
                                )
                            })
